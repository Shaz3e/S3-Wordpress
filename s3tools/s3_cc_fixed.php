...
@media(min-width:980px){
	...
	<?php echo( s3_option('s3cc_link_color') == '' ? '' : '.s3cc-fixed a{ color: ' .s3_option('s3cc_link_color') . '}'); ?>
	<?php echo( s3_option('s3cc_link_hover_color') == '' ? '' : '.s3cc-fixed a:hover{ color: ' .s3_option('s3cc_link_hover_color') . '}'); ?>
}
</style>
<script type="text/javascript">
	$(window).scroll(function(){
		if($(window).scrollTop() + $(window).height() > $(document).height() - <?php echo( s3_option('s3cc_fixed_trigger') == '' ? 1 : s3_option('s3cc_fixed_trigger') );?>) {
